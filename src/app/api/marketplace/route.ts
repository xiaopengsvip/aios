...
export async function GET(req: NextRequest) {
  ...
  if (type === 'all' || type === 'agents') {
    ...
    results.push(...agents.map(a => ({
      ...
      type: 'agents',
      name: a.name,
      description: typeof a.description === 'object' ? (a.description as any)['zh-CN'] || (a.description as any)['en-US'] || '' : String(a.description || ''),
      category: '智能体',
  ...
  if (type === 'all' || type === 'prompts') {
    ...
    results.push(...prompts.map(p => ({
      id: p.id,
      type: 'prompts',
      name: typeof p.title === 'object' ? (p.title as any)['zh-CN'] || '' : String(p.title),
      description: typeof p.description === 'object' ? (p.description as any)['zh-CN'] || '' : String(p.description || ''),
  ...
  if (type === 'all' || type === 'workflows') {
    ...
    results.push(...workflows.map(w => ({
      ...
      type: 'workflows',
      name: w.name,
      description: typeof w.description === 'object' ? (w.description as any)['zh-CN'] || '' : String(w.description || ''),
      category: '工作流',
